...
export const Illuminator: React.FC<{
  ...
  const spotlithtPosition = new Vector3(fireworkLocation.x, fireworkLocation.y, fireworkLocation.z);
  ...
  useFrame(({ camera }) => {
    ...
    if (!isFix) {
      raycaster.setFromCamera(fireworkLocation, camera);
  ...
  return (
    <>
      ...
      <group ref={ref} onClick={fixIlluminator}>
        {lookat.current && <spotLight position={spotlithtPosition} angle={Math.PI / 4} distance={1000} intensity={2} color={color} castShadow shadow-radius={20} shadow-mapSize={[2048, 2048]} />}
        <Sphere args={[5, 32]}>
